...
class LocalAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        padding = Variable(torch.zeros(
            x.size(0), int((self.win_size - 1) / 2), self.embed_size))
...
class DAttn(nn.Module):
    ...
    def __init__(self, input_size, embed_size=100, win_size=5, channels_local=200, channels_global=100,
        ...
        self.localAttentionLayer_user = LocalAttention(
            input_size, embed_size, win_size, channels_local)
        self.globalAttentionLayer_user = GlobalAttention(
            input_size, embed_size, channels_global)
        self.localAttentionLayer_item = LocalAttention(
            input_size, embed_size, win_size, channels_local)
        self.globalAttentionLayer_item = GlobalAttention(
            input_size, embed_size, channels_global)
    ...
    def forward(self, x_user: torch.Tensor, x_item: torch.Tensor):
        # user
        local_user = self.localAttentionLayer_user(x_user)
        global1_user, global2_user, global3_user = self.globalAttentionLayer_user(
            x_user)
        out_user = torch.cat(
            (local_user, global1_user, global2_user, global3_user), 1)
        out_user = out_user.view(out_user.size(0), -1)
        out_user = self.fcLayer(out_user)
        # item
        local_item = self.localAttentionLayer_item(x_item)
        global1_item, global2_item, global3_item = self.globalAttentionLayer_item(
            x_item)
        out_item = torch.cat(
            (local_item, global1_item, global2_item, global3_item), 1)
        out_item = out_item.view(out_item.size(0), -1)
        out_item = self.fcLayer(out_item)
        return torch.sum(torch.mul(out_user, out_item), 1)

# --------------------------------------------------------------------------------------------------------------------------------------------


class LAttOnly(nn.Module):

    def __init__(self, input_size, embed_size=100, win_size=5, channels_local=200,
                 hidden_size=500, output_size=50):
        super(LAttOnly, self).__init__()
        self.localAttentionLayer_user = LocalAttention(
            input_size, embed_size, win_size, channels_local)
        self.localAttentionLayer_item = LocalAttention(
            input_size, embed_size, win_size, channels_local)
        self.fcLayer = nn.Sequential(
            nn.Linear(channels_local, hidden_size),
            nn.Dropout(0.5),
            nn.ReLU(),
            nn.Linear(hidden_size, output_size),
            nn.ReLU(),
    ...
    def forward(self, x_user: torch.Tensor, x_item: torch.Tensor):
        # user
        local_user = self.localAttentionLayer_user(x_user)
        out_user = local_user.view(local_user.size(0), -1)
        out_user = self.fcLayer(out_user)
        # item
        local_item = self.localAttentionLayer_item(x_item)
        out_item = local_item.view(local_item.size(0), -1)
        out_item = self.fcLayer(out_item)
        return torch.sum(torch.mul(out_user, out_item), 1)
...
class GAttOnly(nn.Module):

    def __init__(self, input_size, embed_size=100, win_size=5, channels_global=100,
                 fc_input_size=300, hidden_size=500, output_size=50):
        super(GAttOnly, self).__init__()
        self.globalAttentionLayer_user = GlobalAttention(
            input_size, embed_size, channels_global)
        self.globalAttentionLayer_item = GlobalAttention(
            input_size, embed_size, channels_global)
    ...
    def forward(self, x_user: torch.Tensor, x_item: torch.Tensor):
        ...
        global1_user, global2_user, global3_user = self.globalAttentionLayer_user(
            x_user)
        out_user = torch.cat(
            (global1_user, global2_user, global3_user), 1)
        out_user = out_user.view(out_user.size(0), -1)
        out_user = self.fcLayer(out_user)
        # item
        global1_item, global2_item, global3_item = self.globalAttentionLayer_item(
            x_item)
        out_item = torch.cat(
            (global1_item, global2_item, global3_item), 1)
        out_item = out_item.view(out_item.size(0), -1)
        out_item = self.fcLayer(out_item)
        return torch.sum(torch.mul(out_user, out_item), 1)
